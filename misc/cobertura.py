...
hits_list = [] #later overwritten with data from mongodb for every filename, stores touched lines 
...
class Visitor(ast.NodeVisitor):
    ...
    def line_handler(self, event, stack_event):
        ...
        if hasattr(self.node, 'lineno'):
            ...
            if self.node.lineno not in taken_lines:
                ...
                if self.node.lineno in hits_list:
                    #counter of hits for package/class/method lines
                    line_hits = 1
                    hits_list.remove(self.node.lineno)
    ...
    def exit_class(self, event, stack_event):
        ...
        xml[-1][-1][-1][-1].set('filename', filename)
        ...
        if len(line_set) != 0 and self.total_methods_lines != 0:
            ...
            print "Class zero division!", filename, len(line_set)
            class_line_rate = '0'
